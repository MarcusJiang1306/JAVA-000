CREATE TABLE `test`.`customer` (
  ...
  `address` VARCHAR(54) NOT NULL comment '地址',
...
CREATE TABLE `commodity_info` (
  `commodity_id` INT UNSIGNED NOT NULL AUTO_INCREMENT comment '商品ID' ,
  `commodity_name` VARCHAR(32) NOT NULL comment '商品名称',
  `commodity_type` VARCHAR(32) NOT NULL comment '商品类型',
...
CREATE TABLE `order` (
  ...
  `order_user` VARCHAR(32) NOT NULL comment '下单用户',
   `commodity_id` INT UNSIGNED NOT NULL AUTO_INCREMENT comment '商品ID' ,
  `price` DOUBLE UNSIGNED NOT NULL comment '价格',
  `create_time` BIGINT UNSIGNED NOT NULL comment '创建时间',
  `update_time` BIGINT UNSIGNED NOT NULL comment '更新时间',
